...
def smo_anc_visit(request):
    ...
    #bp_1 = array_append(bp_1, % s), bp_2 = array_append(bp_2, % s),
    #malrepresentation = array_append(malrepresentation, % s), gdm = array_append(gdm, % s), anemia = array_append(
    #    anemia, % s), haemoglobin = array_append(haemoglobin, % s),
    #thyroid = array_append(thyroid, % s), alcohol_tobacco_check = array_append(
    #    alcohol_tobacco_check, % s), preg_related_disease = array_append(preg_related_disease, % s),
    #bleeding_check = array_append(bleeding_check, % s), iugr = array_append(iugr, % s),
    #constant_factors = array_append(constant_factors, % s), variable_factors = array_append(
    ...
    #  const_factors, variable_factors, hrisk_factors, smo_id,visit_number,v_date,advice,
